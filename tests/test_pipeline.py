...
from src.generator import exact_sequences, n_from_order, start_construction
...
def test_start_construction_registry_rejects_unknown_name() -> None:
    assert start_construction("random").name == "random"
    with pytest.raises(ValueError, match="start kind"):
        start_construction("near-turyn")


def test_pipeline_accepts_a_custom_start_construction() -> None:
    class ConstantStart:
        name = "constant"

        def build(self, n: int, rng: np.random.Generator) -> Int8Array:
            del rng
            return np.ones((4, n), dtype=np.int8)
    ...
    result = execute(
        ...
        1,
        42,
        SolverConfig(kick=False, tabu=False),
        ConstantStart(),
    )

    assert result.candidate_evals == 1
    assert np.array_equal(result.sequences, np.ones((4, 5), dtype=np.int8))
...
def test_paley_ng_constructs_verified_order_208() -> None:
    ...
    assert result.candidate_evals == 0
...
def test_construct_searches_smaller_base_then_lifts() -> None:
    result = execute("construct", 14, 2_000_000, 0, SolverConfig())
    assert result.solved
    assert result.candidate_evals > 0
...
def test_construct_fallback_matches_gs4_for_odd_n() -> None:
    config = SolverConfig()
    direct = execute("gs4", 11, 2_000_000, 42, config)
    fallback = execute("construct", 11, 2_000_000, 42, config)
    assert direct.energy == fallback.energy
    assert direct.candidate_evals == fallback.candidate_evals
