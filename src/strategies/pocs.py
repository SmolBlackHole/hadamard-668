...
def _project_weighted_nonperiodic_power(sequences, *, lengths, weights, module=np):
    ...
    power = module.sum(
        module.asarray(weights, dtype=state.dtype).reshape(shape) * module.abs(spectrum) ** 2,
        axis=-2,
    )
    proj = module.fft.ifft(
        spectrum * module.sqrt(target / module.maximum(power, 1e-12))[..., None, :], axis=-1
    ).real[..., : state.shape[-1]]
...
class PocsSearch(TurynStrategy):
    ...
    def _step(self, state, *, module=np):
        ...
        refl = 2 * proj - state
        return (0.5 * (state + 2 * self._project_sign(refl, module=module) - refl)).astype(
            module.float32
        )
    ...
    def search(self, steps: int, seed: int):
        ...
        state = (
            seed_turyn_batch(self.N, batch_size, rng, module=module).astype(module.float32)
            if self.sieve
            else rng.standard_normal((batch_size, 4, self.N)).astype(module.float32)
        )
        state[:, 3, -1] = 0.0
